...
<style>
    ...
    /* 타이틀: 높이를 고정하고 아래 마진 확보 */
    .main-title {{
        text-align: center; font-size: 24px; font-weight: 900; color: #1E3A5F;
        padding: 10px 0; margin-bottom: 5px; line-height: 1.2;
    }}
    
    /* 날짜박스: 상하 여백을 6px로 줄여 슬림하게 유지 */
    .date-box {{ 
        text-align: center; background: #F4F7FF; 
        padding: 6px 10px; border-radius: 15px; font-weight: 800; 
        border: 1px solid #D6DCEC; font-size: 17px; margin: 5px 0; line-height: 1.2; 
    }}
    
    /* 안내 문구: 겹침 방지를 위해 적절한 여백 부여 */
    .status-msg {{ text-align: center; font-size: 13px; font-weight: 700; color: #555; margin: 12px 0; min-height: 20px; line-height: 1.4; }}
    
    .nav-row {{ display: flex; justify-content: space-between; gap: 8px; margin-bottom: 10px; }}
    ...
    .main-menu {{ font-size: 20px; font-weight: 900; color: #111; line-height: 1.5; margin-bottom: 10px; }}
