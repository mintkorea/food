...
def get_realtime_status(selected_meal):
    ...
    for m_name in main_order:
        if now_t < schedule[m_name]["start"]:
            ...
            break
    if next_main:
        diff = datetime.combine(today_date, schedule[next_main]["start"], tzinfo=KST) - get_now()
...
# 7. 스타일 CSS (여백 및 탭 높이 강제 축소)
st.markdown(f"""
<style>
    /* 전체 여백 제거 */
    [data-testid="stAppViewBlockContainer"] {{ 
        max-width: 420px !important; margin: 0 auto !important; 
        padding: 0rem 1rem !important; /* 상단 패딩 제거 */
    }}
    header {{ visibility: hidden; height: 0px; }}
    
    .main-title {{ 
        text-align: center; font-size: 20px; font-weight: 900; color: #1E3A5F; 
        margin-top: -30px !important; /* 타이틀 위로 바짝 올림 */
        margin-bottom: 5px !important;
    }}
    
    .date-box {{ 
        text-align: center; background: #F4F7FF; padding: 3px; border-radius: 12px; 
        font-weight: 800; border: 1px solid #D6DCEC; font-size: 14px; margin-bottom: 5px; 
    }}
    
    .status-msg {{ text-align: center; font-size: 12px; font-weight: 700; color: #555; margin-bottom: 8px; }}
    
    .nav-row {{ display: flex; justify-content: space-between; gap: 5px; margin-bottom: 12px; }}
    .nav-btn {{ flex: 1; text-align: center; padding: 4px 0; background: white; border: 1px solid #EEE; border-radius: 8px; text-decoration: none; color: #1E3A5F; font-size: 11px; font-weight: 800; }}
    
    /* 탭 높이 슬림화 */
    .tab-container {{ display: flex; width: 100%; gap: 1px; }}
    .tab-item {{ 
        flex: 1; text-align: center; 
        padding: 6px 0; /* 높이 결정 패딩 축소 */
        font-size: 11px; font-weight: 800; color: white !important; 
    ...
    /* 카드 테두리 색상 및 여백 */
    .menu-card {{ 
        border: 1.5px solid {sel_c}; border-top: 5px solid {sel_c}; 
        border-radius: 0 0 15px 15px; min-height: 180px; display: flex; flex-direction: column; 
        justify-content: center; align-items: center; padding: 15px; background: white; text-align: center; margin-top: -1px; 
    }}
    .main-menu {{ font-size: 18px; font-weight: 900; color: #111; margin-bottom: 10px; line-height: 1.3; }}
    .side-menu {{ color: #666; font-size: 13px; line-height: 1.4; }}
...
<div class="menu-card">
    ...
    <div style="width:30px; height:1px; background:#EEE; margin-bottom:10px;"></div>
